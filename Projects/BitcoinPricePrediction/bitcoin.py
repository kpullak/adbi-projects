...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
        b = Xi.iloc[i]
        temp = np.exp(wt*similarity(a, (b.iloc[0:(len(b)-1)])))
        denominator = denominator + temp
        numerator = numerator + b[-1]*temp
...
def similarity(a, b):
    ...
    for i in range(0,row_length):
      result = result + ((a.iloc[i] - mean_a)*(b.iloc[i] - mean_b))

    result = result/(row_length*np.std(a)*np.std(b));
    return result;
